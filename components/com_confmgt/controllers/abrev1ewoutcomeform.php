...
class ConfmgtControllerAbrev1ewoutcomeForm extends ConfmgtController
{
	...
	public function edit()
	{
		...
		$this->setRedirect(JRoute::_('index.php?option=com_confmgt&view=abrev1ewoutcomeform&layout=edit', false));
	}
	...
	public function save()
	{
        ...
        if ($return) {
			...
			$outcome = JText::_('CONFMGT_ABSTRACT_REVIEW_OUTCOME_'.$paper->abstract_review_outcome);
			$comments = $paper->abstract_review_comments;
			
			//switch to decide if the comments are to be used as the full email content
			$commentIsEmail = true;
			
			if ($commentIsEmail) {
				$placeholders = array('OUTCOME'=>$outcome, 'PAPERID' => $paperid, 'SITE'=>$sitename );
