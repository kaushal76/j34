	...
	*/ 
	...
	function checkFile($file, $size)
	{
	  ...
	  // No errors, check the file sizes
		  ...
		  //Check for filesize
		  $fileSize = $file['size'];
		  $sizeM = UploadHelper::file_upload_max_size();
		  if($fileSize > $sizeM){
			  JError::raiseWarning( 500, JText::_( 'File exceeded the maximum size ('.$size.'MB)'));
			  return false;
		  }
	  }
	  return true;
	...
	*/ 
	...
	function file_upload_max_size() {
	  ...
	  if ($max_size < 0) {
		...
		$max_size = UploadHelper::parse_size(ini_get('post_max_size'));
	
		// If upload_max_size is less, then reduce. Except if upload_max_size is
		// zero, which indicates no limit.
		$upload_max = UploadHelper::parse_size(ini_get('upload_max_filesize'));
