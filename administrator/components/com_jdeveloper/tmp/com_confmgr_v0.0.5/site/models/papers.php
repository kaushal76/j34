 ...
 * @subpackage  Models
 */
class ConfmgrModelPapers extends JModelList
{
	/**
	 * Constructor.
	 *
	 ...
	 * @return  void
	 */
	protected function populateState($ordering = 'abstract_id', $direction = 'ASC')
	{
		// Get the Application
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__confmgr_paper AS a');				
		...
		if (!empty($search))
		{
			...
			elseif (stripos($search, 'abstract_id:') === 0)
			{
				$search = $db->quote('%' . $db->escape(substr($search, strlen('abstract_id:')), true) . '%');
				$query->where('(a.abstract_id LIKE ' . $search);
			}
			else
			{
