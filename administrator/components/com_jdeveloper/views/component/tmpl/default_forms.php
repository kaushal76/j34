...
<div class="tab-content">
	...
	<?php foreach ($tables as $i => $table) : ?>
	<?php $item_params = $model_table->getParamsFormItem($table->id); ?>
	<?php $item_images = $model_table->getImagesFormItem($table->id); ?>
		...
		<table class="table" id="fieldList<?php echo $table->id ?>">
			...
			<tbody>
				...
				<?php foreach ($model_form->getTable()->getTree($table->form_id) as $field) : ?>
					<?php if ($field->level == "1") continue; ?>
					<?php if (!$table->jfields["params"] && $field->lft >= $item_params->lft && $field->rgt <= $item_params->rgt) continue; ?>
					<?php if (!$table->jfields["images"] && $field->lft >= $item_images->lft && $field->rgt <= $item_images->rgt) continue; ?>
					<tr>
						<td class="nowrap has-context">
							<div class="pull-left">
						...
						<td>
						<?php if (($field->tag == "fields" && $field->name="params") || ($field->tag == "fieldset" && $field->name="params")
								|| ($field->tag == "fields" && $field->name="images") || ($field->tag == "fieldset" && $field->name="images")) : ?>
						<?php else : ?>
