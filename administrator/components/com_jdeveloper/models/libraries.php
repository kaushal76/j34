...
/**
 * JDeveloper Libraries Model
 *
 * @package     JDeveloper
 * @subpackage  Models
 */
class JDeveloperModelLibraries extends JModelList
{
	/**
	 * Constructor
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array('name','a.name','id','a.id');
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			$item->createDir = JDeveloperArchive::getArchiveDir() . "/" . JDeveloperArchive::getArchiveName("lib_", $item->name, $item->version);
			$item->installed = JDeveloperInstall::isInstalled("library", $item->name);
			
			if ($item->installed)
			{
				$extension->load(array("name" => $item->name, "type" => "module"));
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__jdeveloper_libraries AS a');
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__jdeveloper_libraries AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
