...
/**
 * Table class for Camera Ready
 *
 * @package CONFMGT
 *
 * @since version 3.8.0
 */
class ConfmgtTableCameraReady extends JTable {
    ...
    public function bind($array, $ignore = '') {
		...
		if(!empty($files['camera_ready'])){
			...
			if (UploadHelper::checkFile($file, 5)) // file name and file size (in MB) as parameters 
			{
				...
				$extsallowed = explode(',',$params->get('file_type'));
			...
			$filename = UploadHelper::changeFileName($file, 'camera', $array['linkid']);
